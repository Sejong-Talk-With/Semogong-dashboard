...
font = fm.FontProperties(fname='app/DoHyeon-Regular.ttf')
import matplotlib
matplotlib.use('agg')
...
def query_table(db):
    query = '''
            select member_id, Date(create_time), post_times.times from post
            inner join post_times
            on post.post_id = post_times.post_post_id
            where date_sub(now(),interval 6 day) < post.create_time 
            '''
...
def make_plot(result, member_name):
    ...
    fig.savefig('app/static/images/my_plot.png')